...
\hypertarget{section}{%
\section{問題１：両群間計量データの平均値を比較する}\label{section}}

２００名の認知症患者を募集し，認知能力テスト(cognitive test,
COG)，及び脳萎縮の進行度 (brain atrophy,
脳体積の平均年間減少率，単位は\%)
の検査を全員に行った．COG，及び脳萎縮のデータは値が大きいほど認知症の進行度がより進んでいる．また，この２００名の参加者から採取した血液検体を利用して，ある遺伝子の変異の有無を検査した．このデータを以下の表にまとめた：
...
\tightlist
\item
  帰無仮説を「遺伝子変異ありと変異なし両群の間で，COGの平均値は等しい」とする．上記のデータ及び適切な方法を使って検定せよ．検定の結果を分かりやすく説明せよ．なお，分散が等しいと仮定できる場合，以下の式で両群の共通標準偏差が計算できる：
\end{enumerate}
...
\item
  ...
  の比較を 1. と同じ方法で検定してもよいか？ 1.
  と同じ検定方法を使えるかどうかを判断するにはどの検定方法を使えば
  よいかを説明せよ．実際にこの検定方法を行ってください．
\end{enumerate}
...
\subsubsection{ステップ１}\label{section-12}}
...
\textless{}\textless{}\textless{}\textless{}\textless{}\textless{}\textless{}
HEAD 1990年代，アフリカのナイジェリア北部でオンコセルカ症 (回旋糸状虫症,
onchocerciasis; river blindness とも呼ばれる)
が流行していた．ある研究チームは流行地域にある34個の村に住む15歳以上の全住民に目の検査を行った．目の検査受けた住民はWHOの診断基準を元に，「視覚障害
(visually impaired)」と「視力正常 (normal
vision」に分類された．対象者は年間観察され，その期間中に死亡者が登録された．
======= 1990年代，アフリカナイジェリア北部でオンコセルカ症
(回旋糸状虫症, onchocerciasis; river blindness とも呼ばれる)
...
統計量を計算せよ}{上記の２つの表の数字を使って \textbackslash{}chi\^{}2 統計量を計算せよ}}\label{chi2--1}}
...
\paragraph{ステップ１ --- データのインポート}\label{section-39}}
...
\paragraph{ステップ２ --- ロジスティックモデルを作る}\label{section-40}}
...
\item
  ...
  は''視覚障害''と''視力正常''群を比べた log(OR) = log(odds in 視覚障害)
...
\textless{}\textless{}\textless{}\textless{}\textless{}\textless{}\textless{}
HEAD 3. \texttt{agegrp} も説明変数変数へ移動すると自動的に \texttt{+}
が付加される．OKをクリックする． ======= 3. \texttt{agegrp}
も説明変数へ移動すると自動的に \texttt{+}
が入れられる．OKをクリックする．
\textgreater{}\textgreater{}\textgreater{}\textgreater{}\textgreater{}\textgreater{}\textgreater{}
3ca61d9a426ce995501436d6caa88a0536365bd0
...
年齢を考慮していない場合，視覚障害者は視力正常者と比べ，3年間の間に死亡するオッズが5.57倍であり，95\%信頼区間が
3.78 - 8.20 と推定される．
...
と推定される．このオッズ比が大きく変化した（小さくなった）ことは，年齢がこの関連の強い交絡因子であることを示唆している．また，年齢調整したオッズ比の95\%信頼区間は
1
...
\section{問題４：生存分析}\label{section-45}}
...
\tightlist
\item
  視覚障害者（赤線）と視力正常者（黒線）のカプランマイヤー生存曲線：（点線は95\%信頼区間を示す）
\end{enumerate}
