...
２００名の認知症患者を募集し，認知能力テスト(cognitive test, COG)，及び脳萎縮の進行度 (brain atrophy, 脳体積の平均年間減少率，単位は%) の検査を全員に行った．COG，及び脳萎縮のデータは値が大きいほど認知症の進行度がより進んでいる．また，この２００名の参加者から採取した血液検体を利用して，ある遺伝子の変異の有無を検査した．このデータを以下の表にまとめた：
...
1. 帰無仮説を「遺伝子変異ありと変異なし両群の間で，COGの平均値は等しい」とする．上記のデータ及び適切な方法を使って検定せよ．検定の結果を分かりやすく説明せよ．なお，分散が等しいと仮定できる場合，以下の式で両群の共通標準偏差が計算できる：
...
2. この患者データから，遺伝子変異ありとなしの群の間に脳萎縮度 (atrophy) の比較を 1. と同じ方法で検定してもよいか？ 1. と同じ検定方法を使えるかどうかを判断するにはどの検定方法を使えば よいかを説明せよ．実際にこの検定方法を行ってください．
...
- 男の子は女の子と比べて，平均的に体重はどれぐらい多い/少ない？
...
<<<<<<< HEAD
## 年齢，体重それぞれの平均値，分散を求めよ；また，年齢と体重の相関係数を算出せよ．なお，EZRで計量データの平均値を計算するには，コマンド `mean(変数名)` を使う；共分散を計算したい時には，コマンド `cor(変数1, 変数2)` を利用する．
=======
## 年齢，体重それぞれの平均値，分散を求めよ；また，年齢と体重の相関係数を算出せよ．なお，EZRで計量データの平均値を計算するには，コマンド `mean(変数名)` を使う；共分散を計算したい時に，コマンド `cov(変数1, 変数2)` を利用する．
>>>>>>> 3ca61d9a426ce995501436d6caa88a0536365bd0
...
```
推定された回帰係数と自分の計算結果が一致するかを確認してください．
...
- $Y:$ 体重 (kg)；
- $X:$ 年齢 (months)；
- $0.165:$ 子供の年齢が1ヶ月上がると，体重が平均的 0.165 kg (165 g) 高くなる；
...
```{r lm04, cache=TRUE, echo=FALSE, fig.asp=.7, fig.width=8, fig.align='center', out.width='60%'}
...
- $0.164:$ 同じ性別の子供の年齢が1ヶ月上がることによって，体重が平均的に 0.164 kg増える；
...
```{r scatter03, cache=TRUE, echo=FALSE, fig.asp=.7, fig.width=8, fig.align='center', out.height='50%' , out.width='50%'}
...
<<<<<<< HEAD
1990年代，アフリカのナイジェリア北部でオンコセルカ症 (回旋糸状虫症, onchocerciasis; river blindness とも呼ばれる) が流行していた．ある研究チームは流行地域にある34個の村に住む15歳以上の全住民に目の検査を行った．目の検査受けた住民はWHOの診断基準を元に，「視覚障害 (visually impaired)」と「視力正常 (normal vision」に分類された．対象者は年間観察され，その期間中に死亡者が登録された．
=======
1990年代，アフリカナイジェリア北部でオンコセルカ症 (回旋糸状虫症, onchocerciasis; river blindness とも呼ばれる) が流行していた．ある研究チームが流行していた地域の34個の村に住む15歳以上の全住民に目の検査を行った．目の検査受けた住民はWHOの診断基準を元に，「視覚障害 (visually impaired)」と「視力正常 (normal vision」に分類された．対象者は3年間観察され，その期間中に死亡者を登録された．
>>>>>>> 3ca61d9a426ce995501436d6caa88a0536365bd0
...
この検定結果は「視覚障害と対象者の死亡リスクに関連がない」という帰無仮説を棄却するために非常に強い証拠を提供したと言える． (There is strong evidence against the null hypothesis that there is no association between visual impairment and risk of death.)
...
計算したオッズ比はこの結果と一致しているかを確認してください．
...
- `vimp` (視覚障害)の回帰係数 `1.7167` は”視覚障害”と”視力正常”群を比べた log(OR) = log(odds in 視覚障害) - log(odds in 視力正常) = -1.9704 - (-3.689)である．
...
<<<<<<< HEAD
3. `agegrp` も説明変数変数へ移動すると自動的に `+` が付加される．OKをクリックする．
=======
...
knitr::include_graphics("pic/mltlogstic07.png")
```
...
年齢を考慮していない場合，視覚障害者は視力正常者と比べ，3年間の間に死亡するオッズが5.57倍であり，95%信頼区間が 3.78 - 8.20 と推定される．

<<<<<<< HEAD
多変量ロジスティクス回帰モデルを用いて，視覚障害と死亡の関係に年齢の交絡考慮した後，オッズ比が 2.20 になり，95%信頼区間が 1.41 - 3.44 と推定される．このオッズ比が大きく変化した（小さくなった）ことは，年齢がこの関連の強い交絡因子であることを示唆している．また，年齢調整したオッズ比の95%信頼区間は 1 を跨いでいない．以上の結果を踏まえて，「視覚障害者は視力正常者と比べ，観察期間中に死亡するオッズが有意に高いこと」を支持するために，非常に強い証拠を提供した．
=======
...
問題３の研究で，実は対象者が研究に参加した時点と研究終了時点（死亡，打ち切り，または研究期間が終了した）の日付も記録されている：
...
knitr::include_graphics("pic/survival17.png")
```

#### 答え

三年間の追跡期間における，視覚障害者の死亡ハザードは視力正常者の 5.197 倍，このハザード比の95% 信頼区間は 3.56 - 7.51 と推定される．
...
knitr::include_graphics("pic/survival19.png")
```

### 答え

三年間の追跡期間における，視覚障害者の年齢調整死亡ハザードは視力正常者の 2.10 倍，このハザード比の 95% 信頼区間は 1.37 - 3.21 と推定される．
