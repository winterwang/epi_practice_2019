...
2. この患者データから，遺伝子変異ありとなしの群の間に脳萎縮度 (atrophy) の比較を 1. と同じ方法で検定してもよいか？ 1. と同じ検定方法を使えるかどうかを判断するにはどの検定方法を使えばよいかを説明せよ．実際にこの検定方法を行ってください．
...
## 年齢，体重それぞれの平均値，分散を求めよ；また，年齢と体重の相関係数を算出せよ．なお，EZRで計量データの平均値を計算するには，コマンド `mean(変数名)` を使う；共分散を計算したい時には，コマンド `cov(変数1, 変数2)` を利用する．
...
1990年代，アフリカのナイジェリア北部でオンコセルカ症 (回旋糸状虫症, onchocerciasis; river blindness とも呼ばれる) が流行していた．ある研究チームは流行地域にある34個の村に住む15歳以上の全住民に目の検査を行った．目の検査受けた住民はWHOの診断基準を元に，「視覚障害 (visually impaired)」と「視力正常 (normal vision」に分類された．対象者は 3 間観察され，その期間中に死亡者が登録された．
...
EZRで，$\chi^2$統計量と自由度(df)を使って P 値を計算したい場合，以下のコマンドを利用してください：
...
There is strong evidence against the null hypothesis that there is no association between visual impairment and risk of death.
...
- `vimp` (視覚障害)の回帰係数 `1.7167` は”視覚障害”群と”視力正常”群を比べた log(OR) = log(odds in 視覚障害) - log(odds in 視力正常) = -1.9704 - (-3.689)である．
...
- 年齢を考慮していない場合，視覚障害者は視力正常者と比べ，3年の間に死亡するオッズが5.57倍であり，95%信頼区間が 3.78 - 8.20 と推定される．

- 多変量ロジスティクス回帰モデルを用いて，視覚障害と死亡の関係に年齢の交絡考慮した後，オッズ比が 2.20 になり，95%信頼区間が 1.41 - 3.44 と推定される．
